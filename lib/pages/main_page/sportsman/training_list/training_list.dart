  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<QuerySnapshot> snapTraining) {
          ...
          List<Training> trainings = List.from(snapTraining.data!.docs).map((i) => Training.fromDocument(i)).toList();
          return SingleChildScrollView(
            child: ListView.builder(
              ...
              itemBuilder: (_, index) => StreamBuilder(
                  ...
                  builder: (context, AsyncSnapshot<QuerySnapshot> snapExercise) {
                    return TrainingCard(
                      title: trainings[index].name,
                      subtitle: '${snapExercise.data != null ? snapExercise.data!.size : ' '} ${'exercises'.i18n()}',
                      onDelete: () => deletionDialog(context, trainings[index].id),
                      onTap: () async {
                        await Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => TrainingEdit(trainings[index].id!, trainings[index].name)));
                      },
                    );
                  }),
            ),
